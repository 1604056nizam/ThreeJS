// File: src/decimator/decimatePanel.js
export function attachDecimatePanel(opts = {}) {
    const {getSource, onPreview, onDownload, onWireframe, onSwitch, getLabel, getBeforeStats} = opts;
...
`;
    ...
    const refresh = () => {
        ...
        errorVal.textContent = Number(error.value).toFixed(3);

        const before = getBeforeStats?.();
        if (before && Number.isFinite(before.triangles)) {
            stats.textContent = `before: ${before.triangles.toLocaleString()} tris • after: — tris`;
        }
    };
    ...
    $('#dec-switch').addEventListener('click', () => {
        if (typeof onSwitch === 'function') onSwitch('loader');
    });
