...
export function attachRendererTuner({renderer, scene, camera, composer}) {
    ...
    panel.style.cssText = `position:fixed; right:12px; top:12px; z-index:9999; width:280px; padding:10px; border-radius:10px; background:rgba(12,14,22,.9); color:#e8eefc; font:13px/1.25 system-ui; border:1px solid rgba(255,255,255,.08); box-shadow:0 10px 30px rgba(0,0,0,.3);`;
    panel.innerHTML = `
  ...
  `;
    ...
    xr.addEventListener('input', () => {
        ...
        if (renderer.xr.isPresenting) renderer.xr.setFramebufferScaleFactor(xrScale);
    });

    $('#rt-hide').addEventListener('click', () => panel.style.display = 'none');
    window.addEventListener('keydown', (e) => {
        if (e.key.toLowerCase() === 't') panel.style.display = (panel.style.display === 'none') ? 'block' : 'none';
    });
